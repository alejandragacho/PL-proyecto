...
%type <str> secuencia elemento_simple compuesto
...
formula:
    compuesto {
        printf("Fórmula: %s\n", $1);          // Fórmula original de entrada
        printf("Resultado: %s\n\n", $1);     // Resultado procesado
        free($1);                            // Liberar memoria asignada
    }
;
...
compuesto:
    secuencia  /* Compounds are always sequences */
;

elemento_simple:
    ELEMENTO {
        $$ = strdup($1); // Mantén el elemento original
    }
    | ELEMENTO SUBINDICE {
        char original[50];
        sprintf(original, "%s%d", $1, $2); // Ejemplo: H2
        $$ = strdup(original);
    }
;
...
secuencia:
    elemento_simple secuencia {
        char compuesto[200];
        sprintf(compuesto, "%s%s", $1, $2 ? $2 : "");
        $$ = strdup(compuesto);
        free($1);
        if ($2) free($2);
    }
